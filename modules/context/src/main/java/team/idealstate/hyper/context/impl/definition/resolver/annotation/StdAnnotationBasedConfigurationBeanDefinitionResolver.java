...
import team.idealstate.hyper.context.impl.definition.StdBeanScope;
 ...
 * @since 1.0.0
 */
public final class StdAnnotationBasedConfigurationBeanDefinitionResolver implements BeanDefinitionResolver<Configuration> {

    @NotNull
    @Override
    public BeanDefinition resolve(@NotNull Configuration configuration, @NotNull String className) throws UnresolvableBeanDefinitionSourceException {
        AssertUtils.notNull(configuration, "无效的配置 Bean 注解");
        AssertUtils.notNullOrBlank(className, "无效的配置 Bean 类名");
        String name = configuration.value();
        if (StringUtils.isNullOrBlank(name)) {
            name = className;
        }
        return new StdBeanDefinition(name, className, StdBeanScope.LAZY);
    }
}
